<script lang="ts">
	...
	export let charData: CharData | undefined;
	export let charIndex: number;
</script>

{#if !charData}
	<p class="text-slate-500 italic text-center text-base">
...
{:else}
	<div class="mb-4">
		<p class="text-sm font-bold text-slate-600 mb-2 flex items-center">
			...
			<span class="mr-2">({charData.pinyin}, {charData.hanviet})</span>
			<button
				on:click={() => speakChinese(charData.hanzi)}
		...
		<div class="flex flex-wrap gap-2 text-sm">
			{#if charData.words && charData.words.length > 0}
				{#each charData.words as word}
					<button
						on:click={() => speakChinese(word.hanzi)}
					...
					>
						...
						<span class="mr-1">({word.pinyin}, {word.hanviet})</span>
						<svg
							xmlns="http://www.w3.org/2000/svg"
							width="16"
							height="16"
							...
							stroke-linecap="round"
							stroke-linejoin="round"
							class="lucide lucide-volume-1 inline ml-1"
						>
							<polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5" />
